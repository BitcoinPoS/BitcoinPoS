...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BPS additional fees, %d delta bytes\n",
    ...
    else
    {
        ...
        for(size_t i = 0; i < mposScriptList.size(); i++){
            ...
            if(it==vTempVouts.end()){
                return state.Invalid(ValidationInvalidReason::CONSENSUS, error("CheckReward(): A MPoS participant was not properly paid"), REJECT_INVALID, "bad-cs-mpos-missing");
            }else{
                vTempVouts.erase(it);
            }
        }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake() && (pindex->nHeight <= chainparams.GetConsensus().nLastPOWBlock)) {
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-block-type", strprintf("ConnectBlock() : reject proof-of-stake at height %d", pindex->nHeight));
    }

    if (block.IsProofOfWork() && (pindex->nHeight > chainparams.GetConsensus().nLastPOWBlock)) {
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-block-type", strprintf("ConnectBlock() : reject proof-of-work at height %d", pindex->nHeight));
    }
    ...
    if (!CheckReward(block, state, pindex->nHeight, chainparams.GetConsensus(), nFees, nActualStakeReward))
        return state.Invalid(ValidationInvalidReason::CONSENSUS, "block-reward-invalid");
...
bool CChainState::UpdateHashProof(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (block.IsProofOfWork() && nHeight > consensusParams.nLastPOWBlock)
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-block-type", strprintf("UpdateHashProof() : reject proof-of-work at height %d", nHeight));
    
    // Check coinstake timestamp
    if (block.IsProofOfStake() && !CheckCoinStakeTimestamp(block.GetBlockTime()))
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-proof-of-stake-timestamp", strprintf("UpdateHashProof() : coinstake timestamp violation nTimeBlock=%d", block.GetBlockTime()));

    // Check proof-of-work or proof-of-stake
    if (block.nBits != GetNextWorkRequired(pindex->pprev, &block, consensusParams, block.IsProofOfStake()))
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-proof-of-work-or-stake", strprintf("UpdateHashProof() : incorrect %s", block.IsProofOfWork() ? "proof-of-work" : "proof-of-stake"));
...

    {
        ...
        for (size_t i = 0; i < headers.size(); ++i) {
            ...
            if (!fReindex && !fImporting && !::ChainstateActive().IsInitialBlockDownload() && header.IsProofOfStake() && ::StakeSeen().count(std::make_pair(header.prevoutStake, header.nTime)) && !::BlockIndex().count(header.GetHash())) {
                ...
                if(i+1 == headers.size()) {
                    ...
                    if(fInstantBan) {
                        ...
                        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "duplicate-proof-of-stake", strprintf("%s: duplicate proof-of-stake instant ban (%s, %d) for header %s", __func__, header.prevoutStake.ToString(), header.nTime, header.GetHash().ToString()));
                    } else {
                        // otherwise just reject the block until it is part of a longer list
                        return state.Invalid(ValidationInvalidReason::BLOCK_HEADER_REJECT, false, REJECT_INVALID, "duplicate-proof-of-stake", strprintf("%s: duplicate proof-of-stake (%s, %d) for header %s", __func__, header.prevoutStake.ToString(), header.nTime, header.GetHash().ToString()));
                    }
                } else {
            ...
            if (!accepted) {
                ...
                if(fInstantBan) {
                    state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, state.GetRejectReason(), "instant ban, due to duplicate header in the chain");
                }
                return false;
            }
            if (ppindex) {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if(pindex->nHeight > 0){
        BlockMap::iterator mi = ::BlockIndex().find(block.hashPrevBlock);
        if (mi == ::BlockIndex().end())
            return state.Invalid(ValidationInvalidReason::BLOCK_MISSING_PREV, false, REJECT_INVALID, "prev-block-not-found", "AcceptBlock() : prev block not found");
        pindexPrev = (*mi).second;
    }
    ...
    if (block.IsProofOfWork() && nHeight > chainparams.GetConsensus().nLastPOWBlock)
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-proof-of-work", strprintf("AcceptBlock() : reject proof-of-work at height %d", nHeight));

    // Check for the early proof of stake block
    if (block.IsProofOfStake() && nHeight <= chainparams.GetConsensus().nLastPOWBlock)
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-proof-of-stake", strprintf("AcceptBlock() : reject proof-of-stake at height %d", nHeight));
    ...
    if (nHeight >= chainparams.GetConsensus().BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            !std::equal(expect.begin(), expect.end(), block.vtx[0]->vin[0].scriptSig.begin()))
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "block-height-mismatch", "AcceptBlock() : block height mismatch in coinbase");
    }
